...
module.exports = function(config){
    ...
    return {
        ...
        inspectExec: _.flow(({ execId })=> request(`/exec/${execId}/json`), bufferResponse, (promise)=> promise.then(_.flow(bufferToString, JSON.parse, ({ ExitCode, Running })=> ({ exit_code: ExitCode, active: Running })))),
        createContainer: _.flow(({ name, image, cmd, env = {} })=> request('/containers/create', { name }, 'POST', { AttachStdout: true, AttachStderr: true, AttachStdin: true, Env: _.map(env, (v,k)=> [k,v].join('=')), Cmd: _.flatten([cmd]), Image: image, EntryPoint: [] }), bufferResponse, (promise)=> promise.then(_.flow(bufferToString, JSON.parse, ({ Id })=> ({ containerId: Id })))),
